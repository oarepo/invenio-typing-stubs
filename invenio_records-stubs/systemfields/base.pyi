...
class SystemField(ExtensionMixin):
    ...
    # NOTE: the first parameter is named ``record`` (not ``instance``) to match
    # the runtime implementation (``SystemField.__get__(self, record,
    # owner=None)``); otherwise every user-defined field that overrides the
    # descriptor with the runtime parameter names is reported as an invalid
    # override. ``owner`` is kept required on purpose (it is always passed by
    # the descriptor protocol).
    @overload
    def __get__(self, record: None, owner: type[Record]) -> Self: ...
    @overload
    def __get__(self, record: Record, owner: type[Record]) -> Any: ...
    @overload
    def __set__(self, record: None, value: Self) -> None: ...
    @overload
    def __set__(self, record: Record, value: Any) -> None: ...
